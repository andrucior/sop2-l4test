...
void chatWork(int client_socket, char* data, int* client_sockets, int* nrOfClients)
{
    ssize_t  size;
    char name[NAME_SIZE];
    ...
    while(1) {
        ...
        for (int i = 0; i < *nrOfClients; i++)
        {
            if (client_sockets[i] != client_socket) {
                if ((size = bulk_write(client_sockets[i], data, BUFF_SIZE)) < 0 && errno != EPIPE)
                    ERR("write");
            }
        }
    }
                ...
                if (clients < 4) {
                    ...
                    if (size == BUFF_SIZE) {
                        ...
                        if (strcmp(message, key) == 0) {
                            ...
                            switch (fork()) {
                                case 0:
                                    chatWork(client_socket, data, client_sockets, &clients);
                                    return;
                                case -1:
                                    ERR("fork");
                                default:
